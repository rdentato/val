...
tstsuite("Val Library Core Functionality", core) {
    tstcase("Value Creation") {
        tstsection("Basic Types") {
            val_t v1 = val(42);
            tstcheck(valissignedint(v1), "42 should be a signed integer");
            tstcheck(valtosignedint(v1) == 42, "Value should be 42");
            ...
            tstcheck(valisboolean(v3), "true should be a boolean (%016lx)",v3.v);
            tstcheck(valtobool(v3) == true, "Value should be true (%016lx)",v3.v);
            ...
            tstcheck(valisboolean(v4), "false should be a boolean");
            tstcheck(valtobool(v4) == false, "Value should be false (%016lx ->%d)",v4.v,valtobool(v4));
            
                  v4 = val((_Bool)0);
            tstcheck(valisboolean(v4), "false should be a boolean");
            tstcheck(valtobool(v4) == false, "Value should be false (%016lx)",v4.v);
            
            val_t v5 = val(NULL);
            tstcheck(valispointer(v5), "NULL should be a pointer (%016lx)",v5.v);
            tstcheck(valisnullptr(v5), "NULL should be a null pointer (%016lx)",v5.v);
        }
        
        tstsection("Auto Type Selection") {
            // unsigned 
            val_t v6 = val(42u);
            tstcheck(valisunsignedint(v6), "42u should be an unsigned integer");
            ...
            tstcheck(valtopointer(v7) == test_str, "Pointer values should match");
        }
    }
    
    tstcase("Predefined Constants") {
        tstsection("Boolean Constants") {
            tstcheck(valisboolean(valtrue), "valtrue should be a boolean");
            tstcheck(valtobool(valtrue) == true, "valtrue should equal true");
            
            tstcheck(valisboolean(valfalse), "valfalse should be a boolean");
            tstcheck(valtobool(valfalse) == false, "valfalse should equal false");
        }
        
        tstsection("Nil and Null Constants") {
            tstcheck(valisnil(valnil), "valnil should be nil");
            tstcheck(valisnullptr(valnullptr), "valnullptr should be a null pointer");
        }
        
        tstsection("Custom Constants") {
